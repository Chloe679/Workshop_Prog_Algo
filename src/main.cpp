...
sil::Image Mosaique(sil::Image& image){ 
    sil::Image newimage{1000, 1000}; //augmenter car on ne voit pas l'effet sinon
    int sourcex;
    int sourcey;
...
sil::Image MosaiqueMiroir(sil::Image& image){ 
    sil::Image newimage{1000, 1000}; 
    int sourcex;
    int sourcey;
    int miroir=0; // 1 si besoin effet miroir, 0 sinon
     for (int x{0}; x < newimage.width(); x++){
        for (int y{0}; y < newimage.height(); y++){
            ...
            else if ((indX%2==0)&&(indY%2==1)){ //inverse horizontalement
                sourcex=x%image.width();
...
void Glitch(sil::Image& image){
    int x1{}; // coordonnées pixel 1
    int y1{};
    int x2{}; //pixel 2
    int y2{};
    int h{}; //hauteur rectangle
    int l{};//largeur
    int nbrect=random_int(1, 80); //nb rectangles
    for (int i{0};i<nbrect;i++){
        ...
        //on parcourt les deux rectangles pr échanger leurs pixels
        for (int x{0}; x <l; x++){
            for (int y{0}; y < h; y++){
    ...
    for (int x{0}; x < image.width(); x++){
        for (int y{0}; y < image.height(); y++){
            float pourcentage= float(x)/float(image.width() - 1);
             //on crée les vecteurs couleur rouge et vert
...
sil::Image Vortex(sil::Image image) {
    ...
    for (int x = 0; x < image.width(); x++) {
            for (int y = 0; y < image.height(); y++) {
                ...
                angle=dis/sqrt(image.width()*image.width()+image.height()*image.height())*40; //distance sur diago 
                glm::vec2 newpos=rotated(pos,center ,angle);//calcule nv position
                int nx = static_cast<int>(newpos.x); //on transfo en entier les coordonnées de la nv position
